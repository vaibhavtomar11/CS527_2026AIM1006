 ...
 *
 * Lab 3 adds the vector instructions.  A name x0 ... x255 is an integer
 * register, a name v0 ... v31 is a vector register of 8 integer elements.
 */
...
#include <ctype.h>
#include "compiler.h"
#include "processor.h"      /* only for VEC_SCALAR_FLAG, the byte code format */
...
/* is this token a name like <letter><digits> , for example x12 or v3 ? */
static int is_register(char *t, char letter)
{
    int i;

    if (t[0] != letter && t[0] != toupper(letter))
        return 0;
    if (t[1] == '\0')
        return 0;
    for (i = 1; t[i] != '\0'; i++)
...
/* is this token an integer variable like x0 ... x255 ? */
static int is_variable(char *t)
{
    return is_register(t, 'x');
}

/* is this token a vector variable like v0 ... v31 ? */
static int is_vector(char *t)
{
    return is_register(t, 'v');
}
...
/* vector number of v3 is 3 */
static int vector_number(char *t, int line_no)
{
    int n = atoi(t + 1);

    if (n < 0 || n > 31) {
        printf("Compile error (line %d): vector %s is out of range v0-v31\n",
               line_no, t);
        exit(1);
    }
    return n;
...
/* every line which can not be understood ends here */
static void bad_line(int line_no)
{
...
static int pass_two(FILE *in, FILE *out)
{
    ...
    while (fgets(raw, MAX_LINE, in) != NULL) {
        ...
        if (token[0][0] == '[') {
            ...
            if (is_vector(token[4])) {          /* 8 words are written */
                s1 = vector_number(token[4], line_no);
                if (is_variable(token[1])) {
                    d = variable_number(token[1], line_no);
                    emit(out, 0x26, d, s1, 0);
                } else {
                    s2 = constant_value(token[1], line_no);
                    emit(out, 0x2E, 0, s1, s2);
                }
            } else if (is_variable(token[4])) { /* one word is written */
                ...
                printf("Compile error (line %d): only a variable can be "
        ...
        /* ---------- vector destination : v3 = ... ---------- */
        if (is_vector(token[0])) {
            if (count != 5 || token[1][0] != '=') {
                printf("Compile error (line %d): a vector line looks like "
                       "v1 = [x2] , v1 = [32]  or  v1 = v2 + v3\n", line_no);
                exit(1);
            }

            d = vector_number(token[0], line_no);

            /* vector memory read : v1 = [x2]  or  v1 = [32] */
            if (token[2][0] == '[') {
                if (token[4][0] != ']') {
                ...
                if (is_variable(token[3])) {
                    ...
                    emit(out, 0x25, d, s1, 0);
                } else {
                    s2 = constant_value(token[3], line_no);
                    emit(out, 0x2C, d, 0, s2);
                }
                inst_no++;
                continue;
            }

            /* vector arithmetic : v3 = v1 + v4 / x4 / 15 */
            if (!is_vector(token[2])) {
                printf("Compile error (line %d): first operand of a vector "
                       "operation must be a vector\n", line_no);
                exit(1);
            }
            s1 = vector_number(token[2], line_no);
            ...
            switch (token[3][0]) {
                ...
                default:
                    printf("Compile error (line %d): vector operation %s is "
                           "not allowed, only + - *\n", line_no, token[3]);
                    exit(1);
            }

            if (is_vector(token[4])) {
                s2 = vector_number(token[4], line_no);
            } else if (is_variable(token[4])) {
                s2 = variable_number(token[4], line_no);
                s1 = s1 + VEC_SCALAR_FLAG;  /* second operand is x register */
            } else {
