...
void initialize(void)
{
    ...
    n = load_file("program.byte", Instruction, 256);
    if (n == 0) {
        printf("Error: program.byte not found\n");
        exit(1);
    }

    n = load_file("data.byte", Data, 4096);
    if (n == 0)
        printf("Note: data.byte not found, data memory starts with all zero\n");
}
...
void finalize(void)
{
    ...
    if (fp == NULL) {
        printf("Error: can not write data.byte\n");
        return;
    }
    ...
    fclose(fp);
    printf("Data memory written back to data.byte\n");
}
